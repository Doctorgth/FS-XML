
# Third-Party Software Notices / Уведомления о стороннем ПО

This project, **FS-XML Interpreter**, uses third-party open-source libraries. Their licenses and copyright notices are listed below.

---

## 1. PySide6 (Qt for Python)

- **Project:** Qt for Python (PySide6)
- **Copyright:** © The Qt Company Ltd. and other contributors.
- **License:** GNU Lesser General Public License v3 (LGPLv3)
- **Website:** [https://www.qt.io](https://www.qt.io) / [https://wiki.qt.io/Qt_for_Python](https://wiki.qt.io/Qt_for_Python)
- **License Text:** [https://www.gnu.org/licenses/lgpl-3.0.html](https://www.gnu.org/licenses/lgpl-3.0.html)

### Notice / Уведомление:
PySide6 is dynamically linked and imported as an unmodified Python package via `pip`. In compliance with LGPLv3 section 4, users are free to inspect, modify, or replace the PySide6 package in their Python environment without affecting the core application codebase.

---

## 2. Pytest

- **Project:** pytest
- **Copyright:** © Holger Krekel and contributors.
- **License:** MIT License
- **Website:** [https://docs.pytest.org](https://docs.pytest.org)
- **License Text:** [https://opensource.org/licenses/MIT](https://opensource.org/licenses/MIT)
</fs_create>

---
